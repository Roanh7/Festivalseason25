...
// Function to create the MVP votes table if it doesn't exist
async function ensureMvpVotesTable() {
  try {
    ...
    if (!tableExists) {
      console.log('Creating mvp_votes table...');
      await client.query(`
      ...
      `);
      console.log('mvp_votes table created successfully');
    }
  } catch (err) {
    console.error('Error ensuring mvp_votes table exists:', err);
    throw err;
  }
}

// Function to check if a user has voted for a festival's MVP
async function hasUserVotedForMvp(userEmail, festivalName) {
  try {
    await ensureMvpVotesTable();
    
    const result = await client.query(
      'SELECT 1 FROM mvp_votes WHERE user_email = $1 AND festival_name = $2',
      [userEmail, festivalName]
    );
    
    return result.rows.length > 0;
  } catch (err) {
    console.error('Error checking if user voted for MVP:', err);
    throw err;
  }
}

// POST /mvp-vote => submit MVP vote for a festival
app.post('/mvp-vote', async (req, res) => {
  try {
    const { email, festival, mvp } = req.body;
    
    if (!email || !festival || !mvp) {
      return res.status(400).json({ message: 'Missing required fields' });
    }
    
    // Check if user attended the festival
    const attendingResult = await client.query(
      'SELECT 1 FROM attendances WHERE user_email = $1 AND festival_name = $2',
      [email, festival]
    );
    ...
    if (attendingResult.rows.length === 0) {
      return res.status(403).json({ 
        message: 'You can only vote for festivals you attended' 
      });
    }
    ...
    // Check if user already voted for this festival
    const hasVoted = await hasUserVotedForMvp(email, festival);
    
    if (hasVoted) {
      // Update existing vote
      await client.query(
        `UPDATE mvp_votes 
         SET mvp_name = $3, created_at = CURRENT_TIMESTAMP
         WHERE user_email = $1 AND festival_name = $2`,
        [email, festival, mvp]
      );
      
      res.json({ message: 'MVP vote updated successfully' });
    } else {
      // Insert new vote
      await client.query(
        `INSERT INTO mvp_votes (user_email, festival_name, mvp_name)
         VALUES ($1, $2, $3)`,
        [email, festival, mvp]
      );
      
      res.json({ message: 'MVP vote submitted successfully' });
    }
  } catch (err) {
    console.error('Error submitting MVP vote:', err);
    res.status(500).json({ message: 'Could not submit MVP vote' });
  }
});

// GET /mvp-results => get MVP results for all festivals
app.get('/mvp-results', async (req, res) => {
  try {
    // Ensure the table exists
    await ensureMvpVotesTable();
    ...
    // Get MVP results
    const result = await client.query(`
      SELECT mvp_name, COUNT(*) as vote_count
      FROM mvp_votes
      GROUP BY mvp_name
    ...
    console.error('Error getting MVP results:', err);
    res.status(500).json({ message: 'Could not get MVP results' });
  }
});

// GET /festival-mvp/:festival => get MVP results for a specific festival
app.get('/festival-mvp/:festival', async (req, res) => {
  try {
    const festivalName = req.params.festival;
    ...
    // Get MVP results for this festival
    const result = await client.query(`
      SELECT mvp_name, COUNT(*) as vote_count
      FROM mvp_votes
      WHERE festival_name = $1
      GROUP BY mvp_name
      ORDER BY vote_count DESC
    `, [festivalName]);
    ...
    console.error('Error getting festival MVP results:', err);
    res.status(500).json({ message: 'Could not get festival MVP results' });
  }
});

// GET /pending-mvp-votes => get festivals where user hasn't voted for MVP yet
app.get('/pending-mvp-votes', async (req, res) => {
  try {
    const userEmail = req.query.email;
    ...
    // Get festivals user attended but hasn't voted for MVP
    const result = await client.query(`
      SELECT a.festival_name
      FROM attendances a
    ...
    const pendingVotes = result.rows.map(row => row.festival_name);
    
    res.json({ pendingVotes });
  } catch (err) {
    console.error('Error getting pending MVP votes:', err);
    res.status(500).json({ message: 'Could not get pending MVP votes' });
  }
});
...
// Add to notificationScheduler.js: Check for pending MVP votes and send reminders
async function sendMvpVoteReminders() {
  ...
  try {
    ...
    // Ensure the MVP votes table exists
    await ensureMvpVotesTable();
